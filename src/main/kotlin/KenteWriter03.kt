        ...
        program {
            ...
            extend {
                ...
                if (text.length > 0) {
                    ...
                    var x = 0
                    var y = 0
                    for ((index, m) in message.message.withIndex()) {
                        val warpIndex = index % 2
                        val weftIndex = (index / 20) % 2
                        val bit = m % 2
                        if (bit == 0) {
                            rectangles[warpIndex].add(Rectangle(x * 10.0 + 1, y * 10.0, 8.0, 10.0))
                        } else {
                            rectangles[weftIndex + 2].add(Rectangle(x * 10.0, y * 10.0 + 1, 10.0, 8.0))
                        }
                        x++
                        if (x >= 20) {
                            x = 0
